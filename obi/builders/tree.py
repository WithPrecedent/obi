...
Contents:
    Tree (composite.Hybrid, composite.Composite):
    Categorizer (Tree):
    depth_first_search:
...
class Tree(sequence.Hybrid, composite.Composite, abc.ABC):
    ...
    @children.deleter
    def children(self, key: composite.Node) -> None:
        self.delete(key)
        return
           
    """ Dunder Methods """

    def __add__(self, other: composite.Composite) -> None:
        """Adds 'other' to the stored tree using the 'append' method.

        Args:
            other (composite.Composite): another Tree, an adjacency list, an 
    ...
    def __radd__(self, other: composite.Composite) -> None:
...
class Categorizer(Tree):
    ...
    def __add__(self, other: composite.Composite) -> None:
        ...
        Args:
            other (composite.Composite): another Composite or supported
                raw data structure.
