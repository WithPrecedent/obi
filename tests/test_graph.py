...
class Something(obi.Node):
    
    pass


@dataclasses.dataclass
class AnotherThing(obi.Node):
    
    pass


@dataclasses.dataclass
class EvenAnother(obi.Node):
...
def test_graph():
    ...
    matrix = tuple([[[0, 0, 1], [1, 0, 0], [0, 0, 0]],
                    ['scorpion', 'frog', 'river']])
    workflow = obi.System.from_matrix(item = matrix)
    ...
    workflow = obi.System.from_adjacency(item = adjacency)
    ...
    edges = [('camera', 'woman'), 
             ...
             ('person', 'man'), 
             ('tv', 'person')]
    workflow_edges = obi.System.from_edges(item = edges)
