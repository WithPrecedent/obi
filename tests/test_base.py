"""
test_base: tests base classes in obi
...
# def test_proxy():
#     wrapped = TestClass()
#     proxy = obi.Proxy(contents = wrapped)
#     proxy.id = 4543
...
def test_listing():
    listing = obi.Listing(contents = ['a', 'b', 'c'])
    assert listing[1] == 'b'
    listing.add(item = 'd')
    assert listing[3] == 'd'
...
def test_hybrid():
    hybrid = obi.Hybrid(contents = ['a', 'b', 'c'])
...
def test_dictionary():
    alt_created = obi.Dictionary.fromkeys(
        keys = ['a', 'b', 'c'], 
        value = 'tree')
    assert alt_created['a'] == 'tree'
    dictionary = obi.Dictionary(
        contents = {'a': 'b', 'c': 'd'}, 
...
def test_catalog():
    catalog = obi.Catalog(contents = {'tester': TestClass})
...
def test_library():
    library = obi.Library(classes = obi.Catalog(contents = {
        'tester': TestClass}))
